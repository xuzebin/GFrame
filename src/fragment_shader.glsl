...
void main() {
    ...
    vec3 textureNormal = normalize((texture2D(uNormalTexture, vTexCoord).xyz * 2.0) - 1.0);
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        diffuse += max(0.0, dot(textureNormal, lightDirection));
        float attenuation = attenuate(distance(vPosition, uLight[i].lightPosition) / 30.0, 0.5, 0.5);
