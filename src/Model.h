...
public:
    ...
    virtual void setNormalTexture(std::string normalTexName) = 0;
};
...
private:
    ...
    void loadFromFile(const std::string& fileName, std::vector<Vertex>& vertices, std::vector<unsigned short>& indices) {
        ...
        if (materials.size() > 0 && callback != NULL) {
            if (materials[0].diffuse_texname != "") {
                callback->setDiffuseTexture(materials[0].diffuse_texname);
            }
            if (materials[0].specular_texname != "") {
                callback->setSpecularTexture(materials[0].specular_texname);
            }
            if (materials[0].bump_texname != "") {
                callback->setNormalTexture(materials[0].bump_texname);
            }
        }
...
private:
    ...
    void setNormalTexture(std::string normalTexName) {
        ...
        material->normalTexture = loadGLTexture(normalTexName.c_str());
    }
